...
do
  TRANSLATION_FILE="shapefile_encoding_fixer/i18n/$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "shapefile_encoding_fixer/i18n/"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    ${PYLUPDATE} -noobsolete ${PYTHON_FILES} -ts shapefile_encoding_fixer/i18n/${LOCALE}.ts
  done
else
